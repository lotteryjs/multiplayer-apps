# 探索 Golang 高实时、分布式应用服务端编程(精神>思想>技术)

`5G 时代的到临，高实时应用应该会有所爆发……`

Q：一个完整的，较复杂的，可应用于生产的高性能(Golang)实时应用(如：游戏服务器)到底是如何开发出来的呢？
...
* [starx-chat-demo 详解](#快速理解-starx-chat-demo)
  ...
  * `注意: 当前作者估计太忙，所以里面留了两个门槛，注意修改，不然你跑不起来🤦‍♀️`
    * `将 nano.ListenWS 修改为 nano.Listen, 在 nano.Listen 传参里面加上 nano.WithIsWebsocket(true)，不然它就 net.Listen("tcp", n.ClientAddr)，这区别就大了去了🤦‍♀️`
